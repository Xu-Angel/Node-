...
function fetchBrand(req, res) {
  ...
  var reptileMove = function (url, callback) {
    ...
    url = Url.resolve('http://www.autohome.com.cn/grade/carhtml/', url)
    request({
      url: url,
      encoding: null // 关键代码
      ...
      for (var i = 0; i < curBrands.length; i++) {
        ...
        for (var j = 0; j < curSeries.length; j++) {
          var obj = {
            ...
            sub: [],
            url: Url.resolve('http://www.autohome.com.cn/grade/carhtml/',  curSeries.eq(j).attr('href'))
          }
...
function fetchYear(req, res) {
  ...
  var reptileMove = function (series, callback) {
    ...
    request({
      ...
      var html = iconv.decode(body, 'gb2312') // !todo:
      var $ = cheerio.load(html);
